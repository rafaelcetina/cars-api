...
import {
  ...
  ModelResponse,
  UpdateModelDto,
} from '../modules/models/domain/entities/Model'
...
export class ModelController {
  ...
  async getAll(
    ...
    if (_req.query.greater !== undefined) {
      ...
      if (isNaN(greaterThanRequest)) {
        return res
          .status(400)
          .json({ message: 'Invalid query parameter: greater' })
      }
    ...
    if (_req.query.lower !== undefined) {
      ...
      if (isNaN(lessThanRequest)) {
        return res
          .status(400)
          .json({ message: 'Invalid query parameter: lower' })
      }
    ...
    try {
      ...
      return res
        .status(400)
        .json({ message: 'Error getting Models', detail: error.message })
    }
  }
  ...
  async create(req: Request, res: Response): Promise<Response> {
    ...
    try {
      ...
      return res
        .status(400)
        .json({ message: 'Error creating Model', detail: error.detail })
    }
  ...
  async update(_req: Request, _res: Response): Promise<Response> {
    ...
    try {
      ...
      return _res
        .status(400)
        .json({ message: 'Error updating Model', detail: error.detail })
    }
  }
}
